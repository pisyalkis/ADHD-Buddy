...
SKILLS = [
    {
        ...
        "tip": "Нет такого дела, которое ты не мог(ла) бы забыть. Список дел разгружает оперативную память мозга. Как только появилась задача — сразу запиши, не пытайся удержать в голове. Календарь — для встреч с конкретным временем. Список дел — для всего остального."
    },
    {
    ...
    },
    {
        ...
        "tip": "Используй когда: отвлёкся(ась), застрял(а), чувствуешь импульс сделать что-то необдуманное, или просто залип(ла) в телефоне. Пауза разрывает автоматическую реакцию и даёт долю секунды, чтобы выбрать действие осознанно, а не по импульсу. Цель — не изменить поведение, а ЗАМЕТИТЬ что происходит."
    },
    {
    ...
    },
    {
        ...
        "tip": "1. Замети шторм внутри (мысли, чувства, ощущения). 2. Вдави ноги в пол, выпрями спину, сожми пальцы. 3. Найди 5 предметов вокруг, услышь 3-4 звука. Переключение на физические ощущения и звуки вокруг отвлекает мозг от эмоционального шторма и возвращает в настоящий момент. Якорь не уберёт шторм — но удержит тебя в нём."
    },
    {
    ...
    },
    {
        ...
        "tip": "Когда работаешь и приходит отвлекающая мысль — запиши её, скажи себе 'займусь позже' и вернись к задаче. Мозг держит мысль в фокусе, пока боится её потерять — как только она записана, тревога 'не забыть' исчезает, и внимание освобождается для текущей задачи. В конце дня реши: это правда важно, или просто казалось привлекательным?"
    },
    {
        "name": "💧 Холодная вода",
    ...
    },
    {
        ...
        "tip": "Тело и мозг работают в обе стороны: не только настроение влияет на позу, но и поза — на настроение. Расслабленное лицо и полуулыбка сигналят мозгу «сопротивления нет», и внутреннее «не хочу» слабеет. Почувствуй опору под ногами. Расслабь лицо от лба вниз. Мягко приподними уголки губ — это улыбка себе, не на камеру. Ладони вверх. Приступи к задаче. Когда заметишь сопротивление — повтори."
    },
]
