...
PROBLEM_HELP_TEXT = {
    "nostart":    "❓ *Не знаю, с чего начать* — в дневном чекине есть отдельная ветка именно под это, с техникой «Первый неподавляющий шаг»: не планировать всё сразу, а найти только один маленький шаг.",
    "scary":      "😰 *Задачи пугают* — предложу «Готовность и полуулыбка» и «Бросить якорь», чтобы снизить напряжение перед тем как начинать.",
    "resist":     "🧱 *Внутреннее сопротивление* — здесь помогает «Активация» и «Готовность и полуулыбка»: запустить тело раньше, чем появится мотивация.",
    "phone":      "📱 *Залипаю в телефоне* — в дневном чекине есть отдельный вариант для этого, с навыком СТОП и маячками внимания — внешними напоминалками, которые возвращают к задаче.",
    "time":       "⏰ *Не чувствую время* — фокус-таймер задаёт время снаружи, когда изнутри мозг с СДВГ его не чувствует.",
    "memory":     "🧠 *Забываю о делах* — всё, что вносишь в бота, сохраняется в карточке дня и не теряется между утром/днём/вечером, плюс навык «Список дел» — как разгрузить память полностью.",
    "hyperfocus": "🌀 *Гиперфокус* — таймер и маячок работают как внешний прерыватель, который напомнит остановиться, когда сам не заметишь.",
    "emotions":   "🎢 *Резкие эмоции* — «Бросить якорь», дыхание и холодная вода — быстрые техники снижения перевозбуждения, доступны в навыках и подсказываются в моменте.",
}
...
async def onboard_explain_yes(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if lines:
        await q.message.reply_text(
            f"{name}, вот что я буду делать конкретно под твои трудности:\n\n" + "\n\n".join(lines),
            parse_mode="Markdown"
        )
        await asyncio.sleep(0.6)
        await q.message.reply_text(
            "В основе — не мотивационные фразы, а конкретные техники из DBT-тренинга для взрослых с СДВГ "
            "и когнитивно-поведенческой терапии (протокол Safren). Утро — разгон, день — опора, вечер — посадка.",
            parse_mode="Markdown"
        )
    else:
