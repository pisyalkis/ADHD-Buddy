...
async def go_about(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(
        "ℹ️ *О боте*\n\n"
        "Меня сделал человек с СДВГ, который в мае прошёл тренинг навыков и, как многие, "
        "довольно быстро потерялся в рутине и забыл почти всё, чему научился.\n\n"
        "Чтобы держать себя в рамках, он сделал этого бота и засунул сюда плюс-минус всё, "
        "что проходят на тренинге. Каждый день бот напоминает ставить задачи, "
        "предлагает навык дня и помогает, если начинаешь фрустрироваться.\n\n"
        "Мозг с СДВГ живёт интересом и срочностью, а не важностью — "
        "поэтому важное откладывается. Бот держит структуру дня вместо тебя.\n\n"
